...
import { Heart, Search, Bell, MapPin, ChevronDown, X, Check, RefreshCw } from 'lucide-react';
...
export default function BuyerHome() {
  ...
  useEffect(() => {
    ...
    const syncFavs = () => setFavList(getFavorites());
  ...
  const recommended = filteredProperties.slice(0, 4);
  const allProperties = filteredProperties.slice(4);
    ...
    <div className="p-4 pt-12 pb-24 bg-white dark:bg-neutral-950 min-h-screen font-sans relative">
      ...
      <div className="flex items-center gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full bg-gray-100 dark:bg-neutral-900 rounded-full py-3.5 pl-11 pr-4 text-[13px] font-semibold outline-none focus:ring-2 focus:ring-black dark:focus:ring-white placeholder:text-gray-500 text-black dark:text-white"
          />
        </div>
        <button
        ...
        >
          ...
          <span className="absolute top-3 right-3.5 w-2 h-2 bg-red-500 rounded-full border-2 border-gray-100 dark:border-neutral-900"></span>
        </button>
      </div>

      {filteredProperties.length === 0 ? (
        <div className="p-8 text-center bg-gray-50 dark:bg-neutral-900 rounded-3xl border border-dashed border-gray-200 dark:border-neutral-800 my-8">
          ...
          <p className="text-xs text-gray-500 mt-1 mb-4">Try selecting another city or explore all available properties.</p>
          <button
            onClick={() => setSelectedLocation('All Locations')}
            className="px-5 py-2.5 bg-black dark:bg-white text-white dark:text-black rounded-full font-bold text-xs flex items-center gap-1.5 mx-auto shadow-md"
          >
        ...
        <>
          <h2 className="text-lg font-black mb-3 text-black dark:text-white">Recommended for you</h2>
              ...
              return (
                ...
                >
                  <div className="w-full h-32 rounded-2xl bg-gray-100 dark:bg-neutral-900 overflow-hidden relative mb-2">
                    <img
                      src={prop.image_url || 'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9'}
                      alt={prop.title}
                      className="w-full h-full object-cover group-hover:scale-105 transition duration-300"
                    />
                    {idx === 0 && (
                      <span className="absolute top-0 left-0 bg-blue-600 text-white text-[9px] font-bold px-2 py-1 rounded-br-xl">
                        Top Match
                      </span>
                    )}
                    <button
                      onClick={(e) => handleToggleFav(e, prop)}
                      className="absolute top-2 right-2 p-1.5 bg-white/80 dark:bg-black/50 backdrop-blur-md rounded-full active:scale-90 transition"
                    >
                      <Heart size={14} className={fav ? 'fill-red-500 text-red-500' : 'text-gray-700 dark:text-white'} />
                    </button>
                  </div>
          ...
          <h2 className="text-lg font-black mb-3 mt-4 text-black dark:text-white">
            Explore {selectedLocation === 'All Locations' ? 'Everything' : selectedLocation}
          </h2>
          <div className="grid grid-cols-2 gap-3">
            {(allProperties.length > 0 ? allProperties : filteredProperties).map((prop) => {
              const fav = isFavorite(prop.id);
              return (
                <div
                  key={prop.id}
                ...
                >
                  ...
                  <p className="text-[10px] font-semibold text-gray-400 flex items-center gap-0.5 truncate mt-1">
                    <MapPin size={10} /> {prop.location || prop.city}
                  </p>
                </div>
              );
            })}
      ...
      {showLocationSheet && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
          <div className="bg-white dark:bg-neutral-950 w-full max-w-sm rounded-t-[32px] sm:rounded-[36px] p-6 pb-10">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black">Select City</h3>
                ...
                <button
                  ...
                  className={`w-full flex justify-between items-center p-4 rounded-2xl font-bold text-sm transition ${
                    selectedLocation === loc 
                      ...
                      : 'bg-gray-50 text-gray-700 hover:bg-gray-100 dark:bg-neutral-900 dark:text-gray-300'
                  }`}
                >
                  <span>{loc}</span>
